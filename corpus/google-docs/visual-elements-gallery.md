...
| Visible URL | The URL of the page that's shown in a readable format. A visible URL has two parts: domain and breadcrumb. | 
...
| Sitelinks group | Two or more links from the same domain or its localized variations that are clustered together under a text result. For example, the links could be other pages on that domain, headings, or anchors within that page. A sitelinks group contains two or more sitelinks: | 
